...
-- Instances: 247 | Scripts: 4 | Modules: 7 | Tags: 0
local G2L = {};
...
-- StarterGui.Sulfoxide.Remotespymain
G2L["f7"] = Instance.new("ModuleScript", G2L["1"]);
G2L["f7"]["Name"] = [[Remotespymain]];
...
local function codegenerator(remote)
	...
	if remote.method~="OnClientEvent" and remote.method~="OnClientInvoke" then
		if remote.remote.Parent == nil then
			if #remote.args == 0 then
				output = getnil.."\n"..output..'\ngetnil("'..remote.remote.Name..'", "'..remote.remote.ClassName..'", "'..remote.remote.GetDebugId(remote.remote)..'"):'..remote.method..'()'
			else
				output = getnil.."\n"..output..'\ngetnil("'..remote.remote.Name..'", "'..remote.remote.ClassName..'", "'..remote.remote.GetDebugId(remote.remote)..'"):'..remote.method..'(unpack(args))'
			end
			return output	
		end
	...
	elseif remote.method == "OnClientEvent" then
		if remote.remote.Parent == nil then
			if #remote.args == 0 then
				output = getnil.."\n"..output..'\ngetnil("'..remote.remote.Name..'", "'..remote.remote.ClassName..'", "'..remote.remote.GetDebugId(remote.remote)..'"):'..remote.method..'").OnClientEvent)'
			else
				output = getnil.."\n"..output..'\ngetnil("'..remote.remote.Name..'", "'..remote.remote.ClassName..'", "'..remote.remote.GetDebugId(remote.remote)..'")'..'.OnClientEvent, unpack(args))'
			end
			return output
		end
		local fullname = typeenc(remote.remote)
	...
	elseif remote.method == "OnClientInvoke" then
		if remote.remote.Parent == nil then
			if #remote.args == 0 then
				output = getnil.."\n"..'local func = getcallbackvalue(getnil("'..remote.remote.Name..'", "'..remote.remote.ClassName..'", "'..remote.remote.GetDebugId(remote.remote)..'"), "'..remote.method..'")\n'..output.."\nfunc()"
			else
				output = getnil.."\n"..'local func = getcallbackvalue(getnil("'..remote.remote.Name..'", "'..remote.remote.ClassName..'", "'..remote.remote.GetDebugId(remote.remote)..'"), "'..remote.method..'")\n'..output.."\nfunc(unpack(args))"
			end
			return output	
...
script.Parent.Main.group.Remotespy.Logs.examplelog.Parent = nil
...
script.Parent.Main.group.Remotespy.Args.example.Frame.Parent = nil
script.Parent.Main.group.Remotespy.Args.example.Parent = nil
...
script.Parent.Main.group.Remotespy.Buttons.Blockargs.Activated:Connect(function()
	if script.Parent.Main.group.Remotespy.Visible == true and selected then
		getgenv().loggedremotes.blockedremotes["Args"][(selected.remote.GetDebugId(selected.remote))..selected.method] = selected
	end
end)
script.Parent.Main.group.Remotespy.Buttons.Clearlogs.Activated:Connect(function()
	if script.Parent.Main.group.Remotespy.Visible == true then
		for i,v in pairs(script.Parent.Main.group.Remotespy.Logs:GetChildren()) do
			if v:IsA("TextButton") then
				v:Destroy()
			end
		end
		for i,v in pairs(script.Parent.Main.group.Remotespy.Args:GetChildren()) do
			if v:IsA("TextButton") then
				v:Destroy()
			end
		end
...
script.Parent.Main.group.Remotespy.Buttons.Blockremote.Activated:Connect(function()
	if script.Parent.Main.group.Remotespy.Visible == true and selected then
		getgenv().loggedremotes.blockedremotes["All"][(selected.remote.GetDebugId(selected.remote))..selected.method] = selected
	end
end)
...
script.Parent.Main.group.Remotespy.Buttons.Excluderemote.Activated:Connect(function()
	if script.Parent.Main.group.Remotespy.Visible == true and selected then
		getgenv().loggedremotes.ignoredremotes["All"][(selected.remote.GetDebugId(selected.remote))..selected.method] = selected
	end
end)
script.Parent.Main.group.Remotespy.Buttons.Excludeargs.Activated:Connect(function()
	if script.Parent.Main.group.Remotespy.Visible == true and selected then
		getgenv().loggedremotes.ignoredremotes["Args"][(selected.remote.GetDebugId(selected.remote))..selected.method] = selected
	end
end)
script.Parent.Main.group.Remotespy.Buttons.Getfuncinfo.Activated:Connect(function()
	if script.Parent.Main.group.Remotespy.Visible == true and selected and selected:functioninfo() then
...
function addcall(remote)
	...
	if not getgenv().loggedremotes[(remoteinstance.GetDebugId(remoteinstance))..remote.method] then
		local newlog = logexample.Clone(logexample)
		getgenv().loggedremotes[(remoteinstance.GetDebugId(remoteinstance))..remote.method] = {}
		getgenv().loggedremotes[(remoteinstance.GetDebugId(remoteinstance))..remote.method][tonumber(newlog.Frame.calls.Text)+1] = remote
		newlog.Frame.calls.Text = "1"
		newlog.Frame.name.Text = remoteinstance.Name
		newlog.Parent = script.Parent.Main.group.Remotespy.Logs
		newlog.Name = (remoteinstance.GetDebugId(remoteinstance))..remote.method
		newlog.Frame.ImageLabel.Image = ids[remote.method]
		newlog.Visible = not castnumtobool(script.Parent.Main.group.Remotespy.Filters[remote.method].ImageLabel.ImageTransparency)
		newlog.Activated:Connect(function()
			setthreadidentity(7)
			if script.Parent.Main.group.Remotespy.Visible == true then
				if not remote.remote then
					game.GetService(game,"StarterGui").SetCore(game.GetService(game,"StarterGui"), 
						"SendNotification",
						{
							Title = "Error",
							Duration = 3,
				...
				for a,b in pairs(getgenv().loggedremotes[(remoteinstance.GetDebugId(remoteinstance))..remote.method]) do
					local newcall = callexample.Clone(callexample)
					newcall.Parent = script.Parent.Main.group.Remotespy.Args
					newcall.Title.Text = "Call "..tostring(a)
					newcall.Activated:Connect(function() setthreadidentity(7) if script.Parent.Main.group.Remotespy.Visible == true then if lastselectedcall then lastselectedcall.BackgroundTransparency = 1 end newcall.BackgroundTransparency = 0.95; lastselectedcall = newcall; selected = remote end end)
		...
		local remotelog = script.Parent.Main.group.Remotespy.Logs[(remoteinstance.GetDebugId(remoteinstance))..remote.method]
		local newcalls = tostring(tonumber(remotelog.Frame.calls.Text)+1)
		remotelog.Frame.calls.Text = newcalls
		getgenv().loggedremotes[(remoteinstance.GetDebugId(remoteinstance))..remote.method][tonumber(remotelog.Frame.calls.Text)] = remote
